...
class Mimetics(torch.utils.data.Dataset):
    def __init__(self, data_split):
        self.data_split = data_split
        self.data_path = osp.join('/back/dataset/HumanAction/', 'data', 'Mimetics', 'data')
        self.class_num = 50 # 50: a model is trained on Kinetics50, 400: a model is trained on Kinetics400
        self.joint_num = 18
        ...
        self.skeleton = ( (1,0), (1,2), (2,3), (3,4), (1,5), (5,6), (6,7),(1,8), (8,9), (9,10),(1,11), (11,12), (12,13))
        self.flip_pairs = ( (2,5), (3,6), (4,7), (8,11), (9,12), (10,13), (14,15), (16,17) )

        self.datalist = self.load_data()
        
    def load_data(self):
        db = COCO(osp.join(self.data_path, 'Mimetics' + str(self.class_num) + '.json'))
        datalist = []
        for aid in db.anns.keys():
            ...
            img_path = osp.join(self.data_path, 'frames', seq_name, file_name)
            img_height = img['height']; img_width = img['width'];
            frame_num = img['frame_num']

            skeleton_path = osp.join(self.data_path, 'pose_results', seq_name, file_name, 'data.json')
    ...
    def load_skeleton(self, skeleton_path, frame_idxs, original_shape, resized_shape):
        ...
        for i in range(frame_num):
            ...
            if skeleton_data['pose'][i] is None: # human is not detected at this frame
                ...
                pose_coords[idx] = np.ones((cfg.top_k_pose, self.joint_num, 2))
                pose_scores[idx] = np.zeros((cfg.top_k_pose, self.joint_num))
            else:
                pose_coords_per_frame = np.array(skeleton_data['pose'][i]).reshape(-1,self.joint_num,3)
    ...
    def evaluate(self, outs):
        ...
        sample_num = len(annots) # more than annotation number because of cfg.test_infer_num
        ...
        output_path = osp.join(cfg.result_dir, 'mimetics_result.json')
