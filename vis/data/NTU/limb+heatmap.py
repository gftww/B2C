...
def render_limb_gaussian_heatmap(cfg, pose_coord, pose_score):
    x = torch.arange(cfg.input_hm_shape[1]) #[0,1,2, .., 55]
    y = torch.arange(cfg.input_hm_shape[0]) #[0,1,2, ..., 55]
    yy,xx = torch.meshgrid(y,x) #2: H,W
    xx = xx[None,None,None,:,:]; yy = yy[None,None,None,:,:] #5: 1, 1, 1, H,W

    pose_coord = torch.LongTensor(pose_coord)

    # assert skeleton parts
    coord0 = pose_coord[:,:,cfg.skeleton_part[:,0],:] # 4: batch_size*frame_num, person_num, part_num, 2-(x,y)
    coord1 = pose_coord[:,:,cfg.skeleton_part[:,1],:] #  4: batch_size*frame_num, person_num, part_num, 2-(x,y)
    ...
    a_dominate = coeff<=0
    b_dominate = coeff>=1
    seg_dominate =1- a_dominate.byte() - b_dominate.byte()  #5: batch_size*frame_num, person_num, part_num,H,W
    ...
    heatmap = torch.exp(-d2_seg/2./cfg.hm_sigma**2)* (pose_score[:,:,cfg.skeleton_part[:,0],None,None] > cfg.pose_score_thr)
...
if __name__ =="__main__":
    ...
    save_path = "/home/gft/PycharmProjects/vis/data/NTU/figs/S013C001P028R002A058/limb_heatmap"
    ...
    pose_coords, pose_scores = load_skeleton(skeleton_path,frame_idxs,cfg.input_img_shape , cfg.input_hm_shape )

    pose_coords, pose_scores = process_skeleton(cfg, pose_coords, pose_scores,joint_num)

    lim_heatmap = render_limb_gaussian_heatmap(cfg, pose_coords, pose_scores)
    ...
    def heatmap_color(heatmap):
        ...
        for i in range(part_num):
            ...
            heatmap_i = cv2.applyColorMap(heatmap_i.astype(np.uint8),11)

            cv2.imwrite(osp.join(save_path,'heatmap_%s.jpg'%(i)),heatmap_i)

        heatmap_[heatmap_>255]=255
        heatmap_=cv2.applyColorMap(heatmap_.astype(np.uint8),11)
    ...
    heatmap_color(lim_heatmap)
    heatmap_img(lim_heatmap,video[0])
